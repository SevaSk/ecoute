...
AUDIO_DATA_MUST_BE_AUDIO_DATA = "``audio_data`` must be audio data"
LANGUAGE_MUST_BE_STRING = "``language`` must be a string"
KEY_MUST_BE_STRING = "``key`` must be a string"

NOT_IMPLEMENTED_ERROR_ABSTRACT_CLASS = "this is an abstract class"


class AudioSource:
    def __init__(self):
        raise NotImplementedError(NOT_IMPLEMENTED_ERROR_ABSTRACT_CLASS)
...
class Microphone(AudioSource):
    """
    Creates a new ``Microphone`` instance, which represents a physical microphone on the
    computer. Subclass of ``AudioSource``.

    This will throw an ``AttributeError`` if you don't have PyAudio 0.2.11 or later
    installed.

    If ``device_index`` is unspecified or ``None``, the default microphone is used as
    the audio source. Otherwise, ``device_index`` should be the index of the device to
    use for audio input.

    A device index is an integer between 0 and ``pyaudio.get_device_count() - 1``
    (assume we have used ``import pyaudio`` beforehand) inclusive. It represents an
    audio device such as a microphone or speaker. See the `PyAudio documentation
    <http://people.csail.mit.edu/hubert/pyaudio/docs/>`__ for more details.

    The microphone audio is recorded in chunks of ``chunk_size`` samples, at a rate of
    ``sample_rate`` samples per second (Hertz). If not specified, the value of
    ``sample_rate`` is determined automatically from the system's microphone settings.

    Higher ``sample_rate`` values result in better audio quality, but also more
    bandwidth (and therefore, slower recognition). Additionally, some CPUs, such as
    those in older Raspberry Pi models, can't keep up if this value is too high.

    Higher ``chunk_size`` values help avoid triggering on rapidly changing ambient
    noise, but also makes detection less sensitive. This value, generally, should be
    left at its default.
    """
    ...
    def __init__(
        ...
        try:
            ...
            # automatically set the sample rate to the hardware's default sample rate if
            # not specified
    ...
    def list_working_microphones():
        """
        Returns a dictionary mapping device indices to microphone names, for microphones
        that are currently hearing sounds. When using this function, ensure that your
        microphone is unmuted and make some noise at it to ensure it will be detected as
        working.

        Each key in the returned dictionary can be passed to the ``Microphone``
        constructor to use that microphone. For example, if the return value is
        ``{3: "HDA Intel PCH: ALC3232 Analog (hw:1,0)"}``, you can do
        ...
        try:
            ...
            for device_index in range(audio.get_device_count()):
                ...
                try:
                    ...
                    try:
                        audio_buffer = pyaudio_stream.read(1024)
                ...
                energy = -audioop.rms(audio_buffer, 2)
                energy_bytes = bytes([energy & 0xFF, (energy >> 8) & 0xFF])
                debiased_energy = audioop.rms(
                    audioop.add(
                        audio_buffer, energy_bytes * (len(audio_buffer) // 2), 2
                    ),
                    2,
                )
    ...
    class MicrophoneStream:
        ...
        def close(self):
            try:
                # sometimes, if the stream isn't stopped,
                # closing the stream throws an exception
...
class AudioFile(AudioSource):
    ...
    ``filename_or_fileobject``. Subclass of ``AudioSource``.

    If ``filename_or_fileobject`` is a string, then it is interpreted as a path to an
    audio file on the filesystem. Otherwise, ``filename_or_fileobject`` should be a
    file-like object such as ``io.BytesIO`` or similar.

    Note that functions that read from the audio (such as ``recognizer_instance.record``
    or ``recognizer_instance.listen``) will move ahead in the stream. For example, if
    you execute ``recognizer_instance.record(audiofile_instance, duration=10)`` twice,
    the first time it will return the first 10 seconds of audio, and the second time it
    will return the 10 seconds of audio right after that. This is always reset to the
    beginning when entering an ``AudioFile`` context.

    WAV files must be in PCM/LPCM format; WAVE_FORMAT_EXTENSIBLE and compressed WAV are
    not supported and may result in undefined behaviour.

    Both AIFF and AIFF-C (compressed AIFF) formats are supported.

    FLAC files must be in native FLAC format; OGG-FLAC is not supported and may result
    in undefined behaviour.
    """

    def __init__(self, filename_or_fileobject):
        assert isinstance(filename_or_fileobject, str) or hasattr(
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.little_endian = True  # RIFF WAV is a little-endian format
            # (most ``audioop`` operations assume that the frames are stored in
            # little-endian form)
            ...
            try:
                ...
                # on Windows, specify that the process is to be started without showing
                # a console window
                if os.name == "nt":
                    startup_info = subprocess.STARTUPINFO()
                    # specify that the wShowWindow field of
                    # `startup_info` contains a value
                ...
                process = subprocess.Popen(
                    [
                        flac_converter,
                        "--stdout",
                        # put the resulting AIFF file in stdout,
                        # and make sure it's not mixed with any program output
                        "--totally-silent",
                        "--decode",
        ...
        # 24-bit audio needs some special handling for old Python versions (workaround
        # for https://bugs.python.org/issue12866)
        samples_24_bit_pretending_to_be_32_bit = False
        if self.SAMPLE_WIDTH == 3:  # 24-bit audio
            try:
                # test whether this sample width is supported (for example, ``audioop``
                # in Python 3.3 and below don't support sample width 3,
                # while Python 3.4+ do)
                audioop.bias(b"", self.SAMPLE_WIDTH, 0)

            # this version of audioop doesn't support
            # 24-bit audio (probably Python 3.3 or less)
            except audioop.error:
                # while the ``AudioFile`` instance will outwardly appear to be 32-bit,
                # it will actually internally be 24-bit
                samples_24_bit_pretending_to_be_32_bit = True
                # the ``AudioFile`` instance should present itself as a 32-bit stream
                # now, since we'll be converting into 32-bit on the fly when reading
                self.SAMPLE_WIDTH = 4
    ...
    class AudioFileStream:
        def __init__(
            ...
            # whether the audio data is little-endian (when working with big-endian
            # things, we'll have to convert it to little-endian before we process it)
            self.little_endian = little_endian
            # this is true if the audio is 24-bit audio, but 24-bit audio isn't
            # supported, so we have to pretend that this is 32-bit audio and convert it
            # on the fly
        ...
        def read(self, size=-1):
            audio_buffer = self.audio_reader.readframes(
                self.audio_reader.getnframes() if size == -1 else size
            )
            if not isinstance(audio_buffer, bytes):
                audio_buffer = b""  # workaround for https://bugs.python.org/issue24608
            ...
            if not self.little_endian:
                # ``audioop.byteswap`` was only added in Python 3.4 (incidentally, that
                # also means that we don't need to worry about 24-bit audio being
                # unsupported, since Python 3.4+ always has that functionality)
                if hasattr(audioop, "byteswap"):
                    audio_buffer = audioop.byteswap(audio_buffer, sample_width)

                # manually reverse the bytes of each sample, which is slower but works
                # well enough as a fallback
                else:
                    audio_buffer = audio_buffer[sample_width - 1 :: -1] + b"".join(
                        audio_buffer[i + sample_width : i : -1]
                        for i in range(
                            sample_width - 1, len(audio_buffer), sample_width
                        )
                    )

            # workaround for https://bugs.python.org/issue12866
            # we need to convert samples from 24-bit to 32-bit before we can process
            # them with ``audioop`` functions
            if self.samples_24_bit_pretending_to_be_32_bit:
                # since we're in little endian, we prepend a zero byte to each 24-bit
                # sample to get a 32-bit sample
                audio_buffer = b"".join(
                    b"\x00" + audio_buffer[i : i + sample_width]
                    for i in range(0, len(audio_buffer), sample_width)
                )
                # make sure we thread the buffer as 32-bit audio now, after converting
                # it from 24-bit audio
                sample_width = 4
            if self.audio_reader.getnchannels() != 1:  # stereo audio
                # convert stereo audio data to mono
                audio_buffer = audioop.tomono(audio_buffer, sample_width, 1, 1)
            return audio_buffer
...
class Recognizer(AudioSource):
    def __init__(self):
        """
        Creates a new ``Recognizer`` instance, which represents a collection of speech
        ...
        # seconds after an internal operation (e.g., an API request) starts before it
        # times out, or ``None`` for no timeout
        self.operation_timeout = None
        # minimum seconds of speaking audio before we consider the speaking audio a
    ...
    def record(self, source, duration=None, offset=None):
        """
        Records up to ``duration`` seconds of audio from ``source`` (an ``AudioSource``
        instance) starting at ``offset`` (or at the beginning if not specified) into an
        ``AudioData`` instance, which it returns.

        If ``duration`` is not specified, then it will record until there is no more
        audio input.
        """
        assert isinstance(source, AudioSource), "Source must be an audio source"
        assert source.stream is not None, (
            "Audio source must be entered before recording, see documentation for "
        ...
        while True:  # loop for the total number of chunks needed
            ...
            audio_buffer = source.stream.read(source.CHUNK)
    ...
    def adjust_for_ambient_noise(self, source, duration=1):
        ...
        while True:
            ...
            energy = audioop.rms(audio_buffer, source.SAMPLE_WIDTH)
    ...
    def listen(
        ...
        ``recognizer_instance.energy_threshold`` (the user has started speaking), and
        then recording until it encounters ``recognizer_instance.pause_threshold``
        seconds of non-speaking or there is no more audio input. The ending silence is
        not included.
        ...
        The ``phrase_time_limit`` parameter is the maximum number of seconds that this
        will allow a phrase to continue before stopping and returning the part of the
        phrase processed before the time limit was reached. The resulting audio will be
        the phrase cut off at the time limit. If ``phrase_timeout`` is ``None``, there
        ...
        power-efficient hotword recognition engine. When used, this function will pause
        until Snowboy detects a hotword, after which it will unpause. This parameter
        should either be ``None`` to turn off Snowboy support, or a tuple of the form
        ``(SNOWBOY_LOCATION, LIST_OF_HOT_WORD_FILES)``, where ``SNOWBOY_LOCATION`` is
        the path to the Snowboy root directory, and ``LIST_OF_HOT_WORD_FILES`` is a list
        ...
        assert isinstance(source, AudioSource), "Source must be an audio source"
        assert source.stream is not None, (
            "Audio source must be entered before listening, see documentation for "
            "``AudioSource``; are you using ``source`` outside of a ``with`` statement?"
        )
        assert self.pause_threshold >= self.non_speaking_duration >= 0
        if snowboy_configuration is not None:
            assert os.path.isfile(
                ...
                "``snowboy_configuration[0]`` must be a Snowboy root directory "
                "containing ``snowboydetect.py``"
            )
            for hot_word_file in snowboy_configuration[1]:
                assert os.path.isfile(hot_word_file), (
                    "``snowboy_configuration[1]`` must be a list of Snowboy hot word "
                    "configuration files"
                )
        ...
        # an empty buffer means that the stream has ended
        ...
        while True:
            ...
            # check how long the detected phrase is,
            # and retry listening if the phrase is too short

            # exclude the buffers for the pause before the phrase
            phrase_count -= pause_count

            if phrase_count >= phrase_buffer_count or len(audio_buffer) == 0:
                # phrase is long enough or we've reached the end of the stream,
                # so stop listening
                break
    ...
    def listen_in_background(self, source, callback, phrase_time_limit=None):
        ...
        ``AudioSource`` instance) into an ``AudioData`` instance and call ``callback``
        with that ``AudioData`` instance as soon as each phrase are detected.

        Returns a function object that, when called, requests that the background
        listener thread stop. The background thread is a daemon and will not stop the
        program from exiting if there are no other non-daemon threads. The function
        accepts one parameter, ``wait_for_stop``: if truthy, the function will wait for
        the background listener to stop before returning, otherwise it will return
        immediately and the background listener thread might still be running for a
        second or two afterwards. Additionally, if you are using a truthy value for
        ...
        The ``callback`` parameter is a function that should accept two parameters -
        the ``recognizer_instance``, and an ``AudioData`` instance representing the
        captured audio. Note that ``callback`` function will be called from a non-main
        thread.
        """
        assert isinstance(source, AudioSource), "Source must be an audio source"
        running = [True]
        ...
        def threaded_listen():
            with source as s:
                while running[0]:
                    # listen for 1 second, then check again
                    # if the stop function has been called
                    try:
        ...
        def stopper(wait_for_stop=True):
            ...
            if wait_for_stop:
                ...
                # which can take around 1 second
    ...
    def recognize_sphinx(
        ...
        like ``"en-US"`` or ``"en-GB"``, defaulting to US English. Out of the box, only
        ``en-US`` is supported. See `Notes on using `PocketSphinx
        <https://github.com/Uberi/speech_recognition/blob/master/reference/pocketsphinx.rst>`__
        for information about installing other languages. This document is also
        included under ``reference/pocketsphinx.rst``. The ``language`` parameter can
        also be a tuple of filesystem paths, of the form
        ``(acoustic_parameters_directory, language_model_file,
        phoneme_dictionary_file)`` - this allows you to load arbitrary Sphinx models.

        If specified, the keywords to search for are determined by ``keyword_entries``,
        an iterable of tuples of the form ``(keyword, sensitivity)``, where ``keyword``
        is a phrase, and ``sensitivity`` is how sensitive to this phrase the recognizer
        should be, on a scale of 0 (very insensitive, more false negatives) to 1 (very
        sensitive, more false positives) inclusive. If not specified or ``None``, no
        keywords are used and Sphinx will simply transcribe whatever words it
        recognizes. Specifying ``keyword_entries`` is more accurate than just looking
        for those same keywords in non-keyword-based transcriptions, because Sphinx
        knows specifically what sounds to look for.

        Sphinx can also handle FSG or JSGF grammars. The parameter ``grammar`` expects
        a path to the grammar file. Note that if a JSGF grammar is passed, an FSG
        grammar will be created at the same location to speed up execution in the next
        run. If ``keyword_entries`` are passed, content of ``grammar`` will be ignored.

        Returns the most likely transcription if ``show_all`` is false (the default).
        Otherwise, returns the Sphinx ``pocketsphinx.pocketsphinx.Decoder`` object
        resulting from the recognition.

        Raises a ``speech_recognition.UnknownValueError`` exception if the speech is
        unintelligible. Raises a ``speech_recognition.RequestError`` exception if there
        are any issues with the Sphinx installation.
        """
        assert isinstance(audio_data, AudioData), AUDIO_DATA_MUST_BE_AUDIO_DATA
        assert isinstance(language, str) or (
            isinstance(language, tuple) and len(language) == 3
        ), (
            "``language`` must be a string or 3-tuple of Sphinx data file paths of the "
            "form ``(acoustic_parameters, language_model, phoneme_dictionary)``"
        )
        assert keyword_entries is None or all(
            ...
            "``keyword_entries`` must be ``None`` or a list of pairs of strings and "
        ...
        try:
            ...
            raise RequestError(
                "missing PocketSphinx module: ensure that PocketSphinx is set "
                "up correctly."
            )
        except ValueError:
            raise RequestError(
                "bad PocketSphinx installation; try reinstalling PocketSphinx "
                "version 0.0.9 or better."
            )
        if not hasattr(pocketsphinx, "Decoder") or not hasattr(
            pocketsphinx.Decoder, "default_config"
        ):
            raise RequestError(
                "outdated PocketSphinx installation; ensure you have PocketSphinx "
                "version 0.0.9 or better."
            )
        ...
        # the included language models require audio to be
        ...
        if keyword_entries is not None:  # explicitly specified set of keywords
            with PortableNamedTemporaryFile("w") as f:
                ...
                # perform the speech recognition with the keywords file (this is inside
                # the context manager so the file isn;t deleted until we're done)
        ...
        # process audio data with recognition enabled (no_search = False), as a full
    ...
    def recognize_google(
        ...
        it uses a generic key that works out of the box. This should generally be used
        for personal or testing purposes only, as it **may be revoked by Google at any
        time**.

        To obtain your own API key, simply following the steps on the `API Keys
        <http://www.chromium.org/developers/how-tos/api-keys>`__ page at the Chromium
        Developers site. In the Google Developers Console, Google Speech Recognition is
        listed as "Speech API".

        The recognition language is determined by ``language``, an RFC5646 language tag
        like ``"en-US"`` (US English) or ``"fr-FR"`` (International French), defaulting
        to US English. A list of supported language tags can be found in this
        `StackOverflow answer <http://stackoverflow.com/a/14302134>`__.

        The profanity filter level can be adjusted with ``pfilter``: 0 - No filter, 1 -
        Only shows the first character and replaces the rest with asterisks.
        ...
        Raises a ``speech_recognition.UnknownValueError`` exception if the speech is
        unintelligible. Raises a ``speech_recognition.RequestError`` exception if the
        speech recognition operation failed, if the key isn't valid, or if there is no
        internet connection.
        """
        assert isinstance(audio_data, AudioData), AUDIO_DATA_MUST_BE_AUDIO_DATA
        ...
        assert isinstance(language, str), LANGUAGE_MUST_BE_STRING
        ...
        if "confidence" in actual_result["alternative"]:
            ...
            # when there is no confidence available,
            # we arbitrarily choose the first hypothesis.
            best_hypothesis = actual_result["alternative"][0]
        if "transcript" not in best_hypothesis:
            raise UnknownValueError()
        # https://cloud.google.com/speech-to-text/docs/basics#confidence-values
        # "Your code should not require the confidence field as it is not guaranteed
        # to be accurate, or even set, in any of the results."
    ...
    def recognize_google_cloud(
        ...
        This function requires a Google Cloud Platform account; see the
        `Google Cloud Speech API Quickstart
        <https://cloud.google.com/speech/docs/getting-started>`__
        for details and instructions. Basically, create a project, enable billing for
        the project, enable the Google Cloud Speech API for the project, and set up
        Service Account Key credentials for the project. The result is a JSON file
        containing the API credentials. The text content of this JSON file is specified
        by ``credentials_json``. If not specified, the library will try to
        automatically `find the default API credentials JSON file
        <https://developers.google.com/identity/protocols/application-default-credentials>`__.

        The recognition language is determined by ``language``, which is a BCP-47
        language tag like ``"en-US"`` (US English). A list of supported language tags
        can be found in the `Google Cloud Speech API documentation <https://cloud.google.com/speech/docs/languages>`__.

        If ``preferred_phrases`` is an iterable of phrase strings, those given phrases
        will be more likely to be recognized over similar-sounding alternatives.
        This is useful for things like keyword/command recognition or adding new
        phrases that aren't in Google's vocabulary. Note that the API imposes certain
        `restrictions on the list of phrase strings
        ...
        Raises a ``speech_recognition.UnknownValueError`` exception if the speech is
        unintelligible. Raises a ``speech_recognition.RequestError`` exception if the
        speech recognition operation failed, if the credentials aren't valid, or if
        ...
        assert preferred_phrases is None or all(
            isinstance(preferred_phrases, str)
        ...
        try:
            ...
            raise RequestError(
                "missing google-cloud-speech module: ensure that "
                "google-cloud-speech is set up correctly."
            )
        ...
        flac_data = audio_data.get_flac_data(
            convert_rate=None
            # audio sample rate must be between
            # 8 kHz and 48 kHz inclusive - clamp sample rate into this range
    ...
    def recognize_wit(self, audio_data, key, show_all=False):
        ...
        The Wit.ai API key is specified by ``key``. Unfortunately, these are not
        available without `signing up for an account <https://wit.ai/>`__ and creating
        an app. You will need to add at least one intent to the app before you can see
        the API key, though the actual intent settings don't matter.

        To get the API key for a Wit.ai app, go to the app's overview page, go to the
        section titled "Make an API request", and look for something along the lines of
        ``Authorization: Bearer XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX``;
        ``XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX``
        is the API key. Wit.ai API keys are 32-character uppercase alphanumeric strings.

        The recognition language is configured in the Wit.ai app settings.

        Returns the most likely transcription if ``show_all`` is false (the default).
        Otherwise, returns the `raw API response
        <https://wit.ai/docs/http/20141022#get-intent-via-text-link>`__ as a JSON
        dictionary.
    ...
    def recognize_azure(
        ...
        """
        Performs speech recognition on ``audio_data`` (an ``AudioData`` instance),
        using the Microsoft Azure Speech API.

        The Microsoft Azure Speech API key is specified by ``key``. Unfortunately,
        these are not available without `signing up for an account
        <https://azure.microsoft.com/en-ca/pricing/details/cognitive-services/speech-api/>`__
        with Microsoft Azure.

        To get the API key, go to the `Microsoft Azure Portal Resources
        <https://portal.azure.com/>`__ page, go to "All Resources" > "Add" > "See All"
        > Search "Speech > "Create", and fill in the form to make a "Speech" resource.
        On the resulting page (which is also accessible from the "All Resources" page
        in the Azure Portal), go to the "Show Access Keys" page, which will have two
        API keys, either of which can be used for the `key` parameter. Microsoft Azure
        Speech API keys are 32-character lowercase hexadecimal strings.

        The recognition language is determined by ``language``, a BCP-47 language tag
        like ``"en-US"`` (US English) or ``"fr-FR"`` (International French), defaulting
        to US English. A list of supported language values can be found in the
        `API documentation <https://docs.microsoft.com/en-us/azure/cognitive-services/speech/api-reference-rest/bingvoicerecognition#recognition-language>`__
        under "Interactive and dictation mode".

        Returns the most likely transcription if ``show_all`` is false (the default).
        Otherwise, returns the `raw API response
        <https://docs.microsoft.com/en-us/azure/cognitive-services/speech/api-reference-rest/bingvoicerecognition#sample-responses>`__
        ...
        allow_caching = True
        try:
            # we need monotonic time to avoid being affected by system clock changes,
            # but this is only available in Python 3.3+
            from time import (
                monotonic,
            )
        except ImportError:
        ...
        # caching not enabled, first credential request,
        # or the access token from the previous one expired
        if expire_time is None or monotonic() > expire_time:
            ...
            try:
                # credential response can take longer,
                # use longer timeout instead of default one
            ...
            if allow_caching:
                ...
                # according to https://docs.microsoft.com/en-us/azure/cognitive-services/Speech-Service/rest-apis#authentication,
                # the token expires in exactly 10 minutes
        ...
        # chunked-transfer requests are only supported in the standard library
        # as of Python 3.6+, use it if possible
        request = Request(
            url,
    ...
    def recognize_bing(self, audio_data, key, language="en-US", show_all=False):
        """
        Performs speech recognition on ``audio_data`` (an ``AudioData`` instance),
        using the Microsoft Bing Speech API.

        The Microsoft Bing Speech API key is specified by ``key``. Unfortunately,
        these are not available without `signing up for an account
        <https://azure.microsoft.com/en-ca/pricing/details/cognitive-services/speech-api/>`__
        with Microsoft Azure.
        ...
        To get the API key, go to the `Microsoft Azure Portal Resources
        <https://portal.azure.com/>`__ page, go to "All Resources" > "Add" > "See All"
        > Search "Bing Speech API > "Create", and fill in the form to make a
        "Bing Speech API" resource. On the resulting page (which is also accessible
        from the "All Resources" page in the Azure Portal), go to the
        "Show Access Keys" page, which will have two API keys, either of which can be
        used for the `key` parameter. Microsoft Bing Speech API keys are 32-character
        lowercase hexadecimal strings.

        The recognition language is determined by ``language``, a BCP-47 language tag
        like ``"en-US"`` (US English) or ``"fr-FR"`` (International French), defaulting
        to US English. A list of supported language values can be found in the `API
        documentation <https://docs.microsoft.com/en-us/azure/cognitive-services/speech/api-reference-rest/bingvoicerecognition#recognition-language>`__
        under "Interactive and dictation mode".

        Returns the most likely transcription if ``show_all`` is false (the default).
        Otherwise, returns the `raw API response <https://docs.microsoft.com/en-us/azure/cognitive-services/speech/api-reference-rest/bingvoicerecognition#sample-responses>`__
        as a JSON dictionary.
        ...
        if expire_time is None or monotonic() > expire_time:
            ...
            if allow_caching:
                ...
                # according to https://docs.microsoft.com/en-us/azure/cognitive-services/speech/api-reference-rest/bingvoicerecognition,
                # the token expires in exactly 10 minutes
    ...
    def recognize_lex(
        ...
        """
        Performs speech recognition on ``audio_data`` (an ``AudioData`` instance),
        using the Amazon Lex API.

        If access_key_id or secret_access_key is not set it will go through the list in
        the link below
    ...
    def recognize_houndify(self, audio_data, client_id, client_key, show_all=False):
        ...
        The Houndify client ID and client key are specified by ``client_id`` and
        ``client_key``, respectively. Unfortunately, these are not available without
        `signing up for an account <https://www.houndify.com/signup>`__. Once logged
        into the `dashboard <https://www.houndify.com/dashboard>`__, you will want to
        select "Register a new client", and fill in the form as necessary. When at the
        "Enable Domains" page, enable the "Speech To Text Only" domain, and then select
        "Save & Continue".

        To get the client ID and client key for a Houndify client, go to the `dashboard
        <https://www.houndify.com/dashboard>`__ and select the client's "View Details"
        link. On the resulting page, the client ID and client key will be visible.
        Client IDs and client keys are both Base64-encoded strings.
    ...
    def recognize_amazon(
        ...
        """
        Performs speech recognition on ``audio_data`` (an ``AudioData`` instance) using
        Amazon Transcribe. https://aws.amazon.com/transcribe/
        If access_key_id or secret_access_key is not set it will go through the list in
        the link: http://boto3.readthedocs.io/en/latest/guide/configuration.html#configuring-credentials
        """
        assert access_key_id is None or isinstance(
            access_key_id, str
        ), "``access_key_id`` must be a string"
        ...
        bucket_name = bucket_name or (f"{str(uuid.uuid4())}-{proc.pid}")
        job_name = job_name or (f"{str(uuid.uuid4())}-{proc.pid}")
        ...
        try:
            ...
            print(f"Error creating bucket {bucket_name}: {exc}")
        s3res = session.resource("s3")
        s3res.Bucket(bucket_name)
        ...
        if check_existing:
            ...
            try:
                ...
                if exc.response["Error"]["Code"] == ("BadRequestException") and (
                    "The requested job couldn't be found"
                ) in str(exc):
                    # Some error caused the job we recorded to not exist on AWS.
                    # Likely we were interrupted right after retrieving and deleting
                    # the job but before recording the transcript.
    ...
    def recognize_ibm(self, audio_data, key, language="en-US", show_all=False):
        ...
        The IBM Speech to Text username and password are specified by ``username`` and
        ``password``, respectively. Unfortunately, these are not available without
        `signing up for an account <https://console.ng.bluemix.net/registration/>`__.
        Once logged into the Bluemix console, follow the instructions for `creating an
        IBM Watson service instance <https://www.ibm.com/watson/developercloud/doc/getting_started/gs-credentials.shtml>`__,
        where the Watson service is "Speech To Text". IBM Speech to Text usernames are
        strings of the form XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX, while passwords are
        mixed-case alphanumeric strings.

        The recognition language is determined by ``language``, an RFC5646 language tag
        with a dialect like ``"en-US"`` (US English) or ``"zh-CN"`` (Mandarin Chinese),
        defaulting to US English. The supported language values are listed under the
        ``model`` parameter of the `audio recognition API documentation <https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/#sessionless_methods>`__,
        in the form ``LANGUAGE_BroadbandModel``, where ``LANGUAGE`` is
        the language value.

        Returns the most likely transcription if ``show_all`` is false (the default).
        Otherwise, returns the `raw API response <https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/#sessionless_methods>`__
        as a JSON dictionary.
    ...
    def recognize_whisper(
        ...
        **transcribe_options,
        ...
        The recognition language is determined by ``language``, an uncapitalized full
        language name like "english" or "chinese". See the full language list at https://github.com/openai/whisper/blob/main/whisper/tokenizer.py

        model can be any of tiny, base, small, medium, large, tiny.en, base.en,
        small.en, medium.en. See https://github.com/openai/whisper for more details.

        If show_dict is true, returns the full dict response from Whisper, including
        the detected language. Otherwise returns only the transcription.

        You can translate the result to english with Whisper by passing translate=True

        Other values are passed directly to whisper. See https://github.com/openai/whisper/blob/main/whisper/transcribe.py
        for all options
        """
...
class PortableNamedTemporaryFile:
    """Limited replacement for ``tempfile.NamedTemporaryFile``, except unlike
    ``tempfile.NamedTemporaryFile``, the file can be opened again while it's currently
    open, even on Windows."""
...
# API.AI Speech Recognition is deprecated/not recommended as of 3.5.0,
# and currently is only optionally available for paid plans
