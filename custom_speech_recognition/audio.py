...
class AudioData:
    ...
    representing audio samples. This is the frame data structure used by the PCM WAV
    format.
    ...
    The audio data is assumed to have a sample rate of ``sample_rate`` samples per
    second (Hertz).
    ...
    ``recognizer_instance.listen_in_background``, rather than instantiating them
    directly.
    """
    ...
    def get_segment(self, start_ms=None, end_ms=None):
        """
        Returns a new ``AudioData`` instance, trimmed to a given time interval.
        In other words, an ``AudioData`` instance with the same audio data except
        starting at ``start_ms`` milliseconds in and ending ``end_ms`` milliseconds in.

        If not specified, ``start_ms`` defaults to the beginning of the audio, and
    ...
    def get_raw_data(self, convert_rate=None, convert_width=None):
        ...
        If ``convert_rate`` is specified and the audio sample rate is not
        ``convert_rate`` Hz, the resulting audio is resampled to match.

        If ``convert_width`` is specified and the audio samples are not
        ``convert_width`` bytes each, the resulting audio is converted to match.
        ...
        if convert_width is not None and self.sample_width != convert_width:
            ...
            if convert_width == 3:
                ...
                try:
                    # test whether 24-bit audio is supported (for example, ``audioop``
                    # in Python 3.3 and below don't support sample width 3,
                    # while Python 3.4+ does)
                    audioop.bias(b"", 3, 0)

                # this version of audioop doesn't support 24-bit audio (probably
                # Python 3.3 or less)
                except audioop.error:
                    # since we're in little endian, we discard the first byte from each
                    # 32-bit sample to get a 24-bit sample
                    raw_data = b"".join(
        ...
        # if the output is 8-bit audio with unsigned samples, convert the samples we've
    ...
    def get_wav_data(self, convert_rate=None, convert_width=None, nchannels=1):
        """
        Returns a byte string representing the contents of a WAV file containing the
        audio represented by the ``AudioData`` instance.
        ...
        with io.BytesIO() as wav_file:
            ...
            # note that we can't use context manager,
            # since that was only added in Python 3.4
            try:
    ...
    def get_aiff_data(self, convert_rate=None, convert_width=None):
        """
        Returns a byte string representing the contents of an AIFF-C file containing
        ...
        if hasattr(audioop, "byteswap"):
            ...
            # manually reverse the bytes of each sample,
            # which is slower but works well enough as a fallback
    ...
    def get_flac_data(self, convert_rate=None, convert_width=None):
        """
        Returns a byte string representing the contents of a FLAC file containing the
        audio represented by the ``AudioData`` instance.

        Note that 32-bit FLAC is not supported. If the audio data is 32-bit and
        ``convert_width`` is not specified, then the resulting FLAC will be a 24-bit
        FLAC.
        ...
        # resulting WAV data would be 32-bit,
        # which is not convertable to FLAC using our encoder
        if self.sample_width > 3 and convert_width is None:
            # the largest supported sample width is 24-bit,
            # so we'll limit the sample width to that
            convert_width = 3
        ...
        # on Windows, specify that the process is
        # to be started without showing a console window
        if os.name == "nt":
        ...
        process = subprocess.Popen(
            [
                flac_converter,
                "--stdout",
                # put the resulting FLAC file in stdout,
...
def get_flac_converter():
    """Returns the absolute path of a FLAC converter executable, or raises an OSError
    ...
    if flac_converter is None:  # flac utility is not installed
        ...
        if system == "Windows" and machine in {
            ...
            raise OSError(
                "FLAC conversion utility not available - consider installing the "
                "FLAC command line application by running `apt-get install flac` "
